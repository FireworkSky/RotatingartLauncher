...
//            RaLaunchApplication.getVibrationManager().vibrateOneShot(50, 30);
    ...
    init {
        ...
        mMouseClickRunnable = Runnable {
            ...
            if (mIsAttacking && mMouseLeftPressed && mAttackMode == 1 && mInputBridge is SDLInputBridge) {
                ...
                // 发送鼠标左键点击（按下-释放）
                bridge1.sendMouseButton(ControlData.MOUSE_LEFT, true, mouseX, mouseY)
                // 短暂延迟后释放
                mClickAttackHandler.postDelayed(Runnable {
                    if (mIsAttacking && mMouseLeftPressed && mAttackMode == 1) {
                        bridge1.sendMouseButton(ControlData.MOUSE_LEFT, false, mouseX, mouseY)
                    }
                }, 50) // 50ms 按下时间
    ...
    private fun initPaints() {
        ...
        val stickKnobAlpha = if (mData.stickOpacity != 0f) mData.stickOpacity else 1.0f
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        val knobSizeRatio = if (mData.stickKnobSize != 0f) mData.stickKnobSize else 0.5f
        ...
        if (mData.joystickMode == ControlData.JOYSTICK_MODE_MOUSE) {
            ...
            if (mData.xboxUseRightStick) {
                ...
                if (mInputBridge is SDLInputBridge) {
                    ...
                    val currentX = bridge.getVirtualMouseX()
                    val currentY = bridge.getVirtualMouseY()
    ...
    private fun pressDirection(direction: Int) {
        ...
        if (mData.joystickKeys != null) {
            when (direction) {
                DIR_UP -> mInputBridge.sendKey(mData.joystickKeys[0], true)
                DIR_UP_RIGHT -> {
                    mInputBridge.sendKey(mData.joystickKeys[0], true) // W (up)
                    mInputBridge.sendKey(mData.joystickKeys[1], true) // D (right)
                }

                DIR_RIGHT -> mInputBridge.sendKey(mData.joystickKeys[1], true)
                DIR_DOWN_RIGHT -> {
                    mInputBridge.sendKey(mData.joystickKeys[2], true) // S (down)
                    mInputBridge.sendKey(mData.joystickKeys[1], true) // D (right)
                }

                DIR_DOWN -> mInputBridge.sendKey(mData.joystickKeys[2], true)
                DIR_DOWN_LEFT -> {
                    mInputBridge.sendKey(mData.joystickKeys[2], true) // S (down)
                    mInputBridge.sendKey(mData.joystickKeys[3], true) // A (left)
                }

                DIR_LEFT -> mInputBridge.sendKey(mData.joystickKeys[3], true)
                DIR_UP_LEFT -> {
                    mInputBridge.sendKey(mData.joystickKeys[0], true) // W (up)
                    mInputBridge.sendKey(mData.joystickKeys[3], true) // A (left)
                }
    ...
    private fun releaseDirection(direction: Int) {
        ...
        if (mData.joystickKeys != null) {
            when (direction) {
                DIR_UP -> mInputBridge.sendKey(mData.joystickKeys[0], false)
                DIR_UP_RIGHT -> {
                    mInputBridge.sendKey(mData.joystickKeys[0], false)
                    mInputBridge.sendKey(mData.joystickKeys[1], false)
                }

                DIR_RIGHT -> mInputBridge.sendKey(mData.joystickKeys[1], false)
                DIR_DOWN_RIGHT -> {
                    mInputBridge.sendKey(mData.joystickKeys[2], false)
                    mInputBridge.sendKey(mData.joystickKeys[1], false)
                }

                DIR_DOWN -> mInputBridge.sendKey(mData.joystickKeys[2], false)
                DIR_DOWN_LEFT -> {
                    mInputBridge.sendKey(mData.joystickKeys[2], false)
                    mInputBridge.sendKey(mData.joystickKeys[3], false)
                }

                DIR_LEFT -> mInputBridge.sendKey(mData.joystickKeys[3], false)
                DIR_UP_LEFT -> {
                    mInputBridge.sendKey(mData.joystickKeys[0], false)
                    mInputBridge.sendKey(mData.joystickKeys[3], false)
                }
    ...
    private fun startMouseClick() {
        ...
        if (mInputBridge is SDLInputBridge) {
            ...
            val mouseX = bridge.getVirtualMouseX()
            val mouseY = bridge.getVirtualMouseY()


            // Log.v(TAG, "Mouse attack started at (" + mouseX + "," + mouseY + "), mode=" + mAttackMode);
            when (mAttackMode) {
                0, 2 -> bridge.sendMouseButton(ControlData.MOUSE_LEFT, true, mouseX, mouseY)
                1 -> {
                    // 立即发送第一次点击
            ...
            mClickAttackHandler.postDelayed(Runnable {
                if (mIsAttacking) {
                    bridge.sendVirtualTouch(
                        SDLInputBridge.VIRTUAL_TOUCH_RIGHT_STICK,
                        pos[0],
                        pos[1],
                        false
                    )
