...
/**
 * 游戏内 MD3 风格的编辑器设置右侧弹窗
 */
public class GameEditorSettingsDialog {
    ...
    public GameEditorSettingsDialog(Context context, ViewGroup parent, int screenWidth) {
        mParent = parent;
    ...
    private void inflateLayout() {
        ...
        mOverlay = new View(context) {
            ...
            public boolean onTouchEvent(android.view.MotionEvent event) {
                if (event.getAction() == android.view.MotionEvent.ACTION_UP) {
                    ...
                    // 设置按钮在顶部中央，40dp x 40dp，marginTop 1dp
                    float density = getResources().getDisplayMetrics().density;
                    int buttonSize = (int)(40 * density);
                    int buttonTop = (int)(1 * density);
                    int screenCenterX = mScreenWidth / 2;

                    // 检查是否点击了设置按钮区域（留出一些余量以便更容易点击）
                    int touchMargin = (int)(10 * density);
                    if (y >= buttonTop - touchMargin &&
                        y <= buttonTop + buttonSize + touchMargin &&
                        x >= screenCenterX - buttonSize - touchMargin &&
                        x <= screenCenterX + buttonSize + touchMargin) {
                        // 在设置按钮区域，不处理事件，让它传递给下层视图
                        return false;
                    }
        ...
        // 加载对话框布局
        mDialogLayout = (ViewGroup) inflater.inflate(R.layout.dialog_game_editor_settings, mParent, false);
        ...
        mItemExitEditor = mDialogLayout.findViewById(R.id.item_exit_editor);
    ...
    private void setupListeners() {
        ...
        // 退出编辑
        mItemExitEditor.setOnClickListener(v -> {
            if (mListener != null) {
                mListener.onExitEditor();
            }
        });
    }
}
