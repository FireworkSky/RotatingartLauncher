...
internal class StartupHook
{
    ...
    {
        Console.WriteLine("[StartupHook] PlatformFixPatch DOTNET_STARTUP_HOOKS executing...");
        
        // 注册程序集解析器,从 monomod 目录加载依赖
        AppDomain.CurrentDomain.AssemblyResolve += OnAssemblyResolve;

        // 调用补丁初始化方法
        int result = PlatformFixPatch.PlatformFixPatcher.Initialize(IntPtr.Zero, 0);
        Console.WriteLine($"[StartupHook] PlatformFixPatcher.Initialize returned: {result}");
    }
