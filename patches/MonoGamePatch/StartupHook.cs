...
/// 
/// 重要: 此补丁必须在其他补丁之前加载，以便在 MonoGame 初始化之前修复原生库加载
/// </summary>
internal class StartupHook
{
    ...
    {
        Console.WriteLine("[StartupHook] MonoGamePatch DOTNET_STARTUP_HOOKS executing...");
        
        // 注册程序集解析器,从 monomod 目录加载依赖
        AppDomain.CurrentDomain.AssemblyResolve += OnAssemblyResolve;

        // 调用补丁初始化方法
        int result = MonoGamePatch.MonoGamePatcher.Initialize(IntPtr.Zero, 0);
        Console.WriteLine($"[StartupHook] MonoGamePatcher.Initialize returned: {result}");
    }
